...
const ProductCard: React.FC<Props> = ({ product, stock, onDecrease }) => {
  ...
  return (
    <div className="p-4 mr-3  rounded shadow border bg-white">
      <img
        src={product.image}
        alt={product.title}
      ...
      <p className="text-xs text-gray-500 italic mb-1 capitalize">
        {product.category}
      </p>
      ...
      <button
        className="mt-2 block w-full bg-gray-200 hover:bg-gray-300 text-sm py-1.5 rounded"
        onClick={onDecrease}
      >
        Decrease Stock
      </button>
